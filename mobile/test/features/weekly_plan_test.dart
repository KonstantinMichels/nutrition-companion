...
void main() {
  test('weekly model preserves Monday-to-Sunday order and planned-day average', () {
    final week = WeeklyPlan.fromJson({
      ...
      'days': List.generate(7, (index) => {
        'date': DateTime(2026, 7, 27 + index).toIso8601String().substring(0, 10),
        'weekday': const ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'][index],
        'state': index == 0 ? 'planned' : 'no_plan',
        'plan': index == 0 ? {'id': 'plan-1'} : null,
        'summary': {'meal_count': index == 0 ? 1 : 0, 'entry_count': index == 0 ? 1 : 0},
        'warnings': <Object>[],
        'meals': <Object>[],
      }),
      ...
      'quality': {'quality_level': 'partial_week', 'mixed_assessment_basis': false},
      'warnings': <Object>[],
    });
  ...
  testWidgets('navigation exposes weekly planning without automatic planning or score', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: AppScaffold(title: 'Test', body: SizedBox.shrink())),
    );
